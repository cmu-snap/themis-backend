...
class ExperimentAnalyzer:
    ...
    def _create_queue_hdfstore(self, queue_log_localpath, f):
        with pd.HDFStore(queue_log_localpath, mode='w') as store:
            previous_last_row = pd.DataFrame() # queue-tmp.txt
            for chunk in pd.read_csv(
                    ...
                    dtype={'dequeued': bool, 
                           ...
                           'batch':np.uint16},
                    chunksize=100000):
                chunk['seq'] = chunk['seq'].astype(np.uint32)
                chunk['src'] = chunk['src'].astype(np.uint16)
                #chunk['time'] = pd.to_datetime(chunk['time'], infer_datetime_format=True, unit='ns')
                chunk['lineno'] = chunk.index + 1 
                chunk = chunk.set_index('time').sort_index() 
                df_enq = (pd
                          .get_dummies(chunk[(chunk.dequeued==0) & (chunk.dropped==0)]['src'])
                          .astype(np.uint8))
                df_deq = (pd
                          .get_dummies(chunk[chunk.dequeued==1]['src'])
                          .replace(1,-1)
                          .astype(np.int8))
                if not previous_last_row.empty:
                    df_flows = (df_enq
                                .append(df_deq)
                                .append(previous_last_row)
                                .sort_index()
                                .cumsum()
                                .fillna(0)
                                .astype(np.uint32))
                    df_flows = df_flows.iloc[1:]
                else:
                    df_flows = (df_enq
                                .append(df_deq)
                                .sort_index()
                                ...
                                .fillna(0)
                                .astype(np.uint32))
                previous_last_row = df_flows.tail(1)
                df = (chunk
                                          .reset_index()
    ...
    def df_queue(self):
        ...
        if (self._df_queue is None) or (not os.path.isfile(queue_log_localpath)):
            ...
            if not os.path.isfile(queue_log_localpath):
                raw_queue_log_localpath = os.path.join(DATAPATH_RAW,
                                                       queue_log_tarpath)
                if not os.path.isfile(raw_queue_log_localpath):
                    with untarfile(self.experiment.tarfile_localpath,
                                   queue_log_tarpath) as f:
                        # skip rows that are not for flows we care about
                        ...
                        completed_proc = subprocess.run([
                            ...
                            raw_queue_log_localpath], check=True)
                        self._create_queue_hdfstore(queue_log_localpath,
                                                    f)
                else:
                    completed_proc = subprocess.run([
                        'sort', 
                        ...
                        raw_queue_log_localpath,
                        raw_queue_log_localpath], check=True)
                    with open(raw_queue_log_localpath) as f:
                        self._create_queue_hdfstore(queue_log_localpath,
                                                    f)                        
                #else:
                ...
                #rename_src_ports = {src_hex:int(str(src_hex), 16) for src_hex in df.src.unique()}t
                #df = df.rename(columns = rename_src_ports)
                #df.to_csv(queue_log_localpath, header=True)
                #self._df_queue = df
            #else:
            #store = pd.HDFStore(queue_log_localpath)
            #df = pd.read_hdf(queue_log_localpath)
                    #df.index = pd.to_datetime(df.index)
                    #df['src'] = df['src'].astype('str')
            self._df_queue = queue_log_localpath
