...
// ADDED by RAY from tcp_bbr.c
/* BBR congestion control block */
struct bbr {
	u32	min_rtt_us;	        /* min RTT in min_rtt_win_sec window */
	u32	min_rtt_stamp;	        /* timestamp of min_rtt_us */
	u32	probe_rtt_done_stamp;   /* end time for TCP_MEASURE_PROBE_RTT mode */
	struct minmax bw;	/* Max recent delivery rate in pkts/uS << 24 */
	u32	rtt_cnt;	    /* count of packet-timed rounds elapsed */
	u32     next_rtt_delivered; /* scb->tx.delivered at end of round */
	u64	cycle_mstamp;	     /* time of this cycle phase start */
	u32     mode:3,		     /* current tcp_measure_mode in state machine */
		...
		probe_rtt_round_done:1,  /* a TCP_MEASURE_PROBE_RTT round at 4 pkts? */
		unused:5,
	...
	if (((port == 0 && fwmark == 0) ||
		...
		if (tcp_probe_avail() > 1) {
			...
			if (!(strcmp(ca_ops->name, "bbr")) || !(strcmp(ca_ops->name, "bbr_ray"))) {
			  ...
			  u64 bw = bbr_bw(sk);
			  p->bbr_bw_lo = (u32)bw;
			  p->bbr_bw_hi = (u32)(bw >> 32);
			  p->bbr_min_rtt = bbr->min_rtt_us;
...
static int tcpprobe_sprint(char *tbuf, int n)
{
	...
	return scnprintf(tbuf, n,
			...
			p->snd_cwnd, p->ssthresh, p->snd_wnd, p->srtt, p->rcv_wnd,
			 p->bbr_bw_lo, p->bbr_bw_hi, p->bbr_min_rtt,
			 p->bbr_pacing_gain, p->bbr_cwnd_gain);
}
