...
rtt_diffs = [1+.25, 1-.25,1+.5,1-.5,1+.75,1-.75,1+1]
LOCAL_EXPS_RTT = {}
for bw, rtt, q in NTWRK_CONDITIONS:
    for cca in CCAS:
        for rtt_diff in rtt_diffs:
            testing_exp = '{}-{}bw-{}rtt-{}q'.format(cca, bw, int(rtt*rtt_diff), q)
            training_exps = '{}bw-{}rtt-{}q'.format(bw, rtt, q)
            assert(testing_exp not in LOCAL_EXPS_RTT)
            for exp in glob.glob('/opt/cctestbed/data-raw/{}-local-*.tar.gz'.format(testing_exp)):
                LOCAL_EXPS_RTT[os.path.basename(exp)[:-7]] = training_exps

#TESTING_EXPS_DICT = {'{}-{}bw-{}rtt-{}q-local'.format(cca, bw, rtt, q): glob.glob(
#    'data-raw/{}-{}bw-{}rtt-{}q-local-*.tar.gz'.format(cca, bw, rtt, q))
#                   for bw, rtt, q in NTWRK_CONDITIONS for cca in CCAS}
TESTING_EXP_NAMES = list(LOCAL_EXPS_RTT.keys())
...
# reusable
EXP_NAMES=TESTING_EXP_NAMES
CSV_OUTPUT_NAME='data-processed/classify-rtt-exps.csv'
...
def get_local_exps(wildcards):
    ...
    ntwrk_conditions = LOCAL_EXPS_RTT[wildcards.exp_name]
    #re.match('.*-(\d+bw-\d+rtt-\d+q).*',
